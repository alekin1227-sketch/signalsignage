...
export function Devices(){
  ...
  return <>
    <div className="page-heading"><div><span className="eyebrow">Rede de exibição</span><h1>Dispositivos</h1><p>{count} de {devices.length} TVs conectadas à central neste momento.</p></div><Button variant="outline" onClick={load}><RefreshCw size={16}/>Atualizar rede</Button></div>
    <section className="device-grid">{devices.map(device=><Card key={device.id} className="device-card">
      <div className="device-screen p-5"><div className="flex justify-between"><Monitor/><Badge online={online(device)}>{online(device)?'Online':'Offline'}</Badge></div><div className="mt-12"><p className="text-[9px] font-bold tracking-[.16em] text-slate-400">EM EXIBIÇÃO</p><p className="mt-1 truncate text-sm font-semibold">{device.nowPlayingName||'Nenhuma mídia informada'}</p></div></div>
