...
export function MediaPage(){
  ...
  async function uploadFiles(selected:FileList|File[]){
    ...
    for(let index=0;index<files.length;index+=1){const file=files[index];setProgress(`Processando ${index+1} de ${files.length}: ${file.name}`);const data=new FormData();data.append('file',file);try{await api('/media/upload',{method:'POST',body:data});completed+=1}catch(event){failures.push(`${file.name}: ${(event as Error).message}`)}}
  ...
  return <>
    <div className="mb-7"><h1 className="text-3xl font-bold">Mídias</h1><p className="text-slate-500">Envie imagens, vídeos e PDFs ou incorpore uma página externa. APIs e notícias são criadas em Widgets de dados.</p></div>
    ...
    <div className="mb-6 grid gap-4 xl:grid-cols-3">
      <Card className="p-5 xl:col-span-2"><h2 className="mb-1 font-semibold">Adicionar arquivos</h2><p className="mb-3 text-xs text-slate-500">Arraste arquivos ou clique para escolher. Imagens e vídeos são normalizados automaticamente; aceita também PDF.</p><label onDragEnter={event=>{event.preventDefault();setDragging(true)}} onDragOver={event=>event.preventDefault()} onDragLeave={event=>{event.preventDefault();setDragging(false)}} onDrop={drop} className={`grid min-h-44 cursor-pointer place-items-center rounded-xl border-2 border-dashed p-6 text-center transition ${dragging?'border-primary bg-primary/10':'border-slate-300 hover:border-primary dark:border-slate-700'}`}><input ref={inputRef} className="hidden" type="file" multiple accept="image/*,video/*,application/pdf,.mkv,.avi,.mov,.wmv,.m4v,.webm,.mpeg,.mpg,.ts,.mts,.m2ts,.3gp,.ogv,.flv,.heic,.heif,.tif,.tiff,.svg,.avif,.bmp" onChange={event=>event.target.files&&void uploadFiles(event.target.files)} disabled={busy}/><span><UploadCloud className="mx-auto mb-3 text-primary" size={34}/><strong className="block">{busy?'Enviando e preparando arquivos…':'Solte os arquivos aqui'}</strong><small className="mt-1 block text-slate-500">ou clique para escolher um ou vários ficheiros</small>{progress&&<small className="mt-3 block font-medium text-primary">{progress}</small>}</span></label></Card>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{media.map(item=><Card key={item.id} className="overflow-hidden"><div className="grid aspect-video place-items-center bg-muted">{item.thumbnailUrl?<img className="h-full w-full object-cover" src={`${API_URL.replace(/\/api$/,'')}${item.thumbnailUrl}`} alt=""/>:item.type==='VIDEO'?<FileVideo size={40}/>:item.type==='PDF'?<FileText size={40}/>:item.type==='URL'?<ExternalLink size={40}/>:item.type==='FEED'?<Newspaper size={40}/>:item.type==='WIDGET'?<LayoutDashboard size={40}/>:<Image size={40}/>}</div><div className="flex items-center gap-2 p-4"><div className="min-w-0 flex-1"><p className="truncate font-medium">{item.name}</p><p className="text-xs text-slate-500">{item.type==='WIDGET'?`Widget · ${item.feedRefreshSec??300}s`:item.type==='FEED'?`Feed · ${item.feedRefreshSec??300}s`:item.type==='VIDEO'?`Vídeo${item.durationSec?` · ${item.durationSec}s`:''}`:item.type==='PDF'?'PDF':item.type}</p></div><Button variant="ghost" title="Excluir mídia" onClick={()=>remove(item)}><Trash2 size={17}/></Button></div></Card>)}</section>
  </>;
}
